...
class FarmasiController extends Controller
{
  ...
  public function data(Request $request)
  {
    ...
    if (empty($request->input('search.value'))) {
      $obat = Obat::daftar_obat('', $start, $limit, $order, $dir);
    } else {
      $search = $request->input('search.value');
      $obat = Obat::daftar_obat($search, $start, $limit, $order, $dir);
      $totalFiltered = Obat::total_daftar_obat($search);
  ...
  {
    $data['obat'] = Obat::detail_obat($id_obat);
    $data['stok'] = HistoryObat::stok_obat($id_obat);
    return view('farmasi/transaksi', $data)->render();
  }

  public function tambah_transaksi($id_obat = null)
  {
    $data['obat'] = Obat::daftar_obat_all();
  ...
  public function edit_transaksi($id_history)
  {
    ...
    $data['obat'] = Obat::detail_obat($data['history']->id_obat);
  ...
  public function transaksi_data(Request $request)
  {
    ...
    $obat = HistoryObat::histori($search, $id_obat, $start, $limit, $order, $dir, $startDate, $endDate);
    $totalData = HistoryObat::histori_count($search, $id_obat)->total;
    $totalFiltered = $totalData;
    $data = array();
    foreach ($obat as $key => $p) {
      // $penghuni = Penghuni::detail_penghuni($p->id_penghuni);
  ...
  public function proses_hapus_transaksi(Request $request)
  {
    ...
    HistoryObat::delete_transaksi($id);
