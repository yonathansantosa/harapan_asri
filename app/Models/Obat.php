...
class Obat extends Model
{
  ...
  {
    if ($query == "") {
      $data = Obat::selectRaw('tb_obat.id, tb_obat.kode_slug, penghuni.nama, tb_obat.namaobat, sum(tb_history_obat.stokobat) as stok')
        ->leftJoin('tb_history_obat', 'tb_history_obat.id_obat', '=', 'tb_obat.id')
        ->leftJoin('penghuni', 'penghuni.id_penghuni', 'tb_history_obat.id_penghuni')
        ...
        ->limit($limit)
        ->orderBy($order, $dir)
        ->groupBy('tb_history_obat.id_obat', 'penghuni.nama', 'tb_obat.id', 'tb_obat.kode_slug', 'tb_obat.namaobat')
        ->get();
    } else {
      $data = Obat::selectRaw('tb_obat.id, tb_obat.kode_slug, penghuni.nama, tb_obat.namaobat, sum(tb_history_obat.stokobat) as stok')
        ->leftJoin('tb_history_obat', 'tb_history_obat.id_obat', '=', 'tb_obat.id')
